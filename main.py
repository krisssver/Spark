...
def agg_calc(spark: SparkSession) -> DataFrame:
    ...
    datamart = trip_fact \
        ...
        .agg(f.avg(trip_fact['total_amount']).alias("avg_trip_coast"), (f.sum(trip_fact['total_amount'])/f.sum(trip_fact['trip_distance']*1.61)).alias('avg_trip_km_coast')) \
        .select(f.col('dt'),
                f.col('payment_type'),
                f.col('avg_trip_coast'),
                f.col('avg_trip_km_coast')) \
